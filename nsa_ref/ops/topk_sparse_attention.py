...
def forward_kernel_split_k(
    ...
    # Process pid 0
    pid_0 = tl.program_id(0) # [0, batch_size * num_k_heads * max_seqlen_q)
    pid_batch = pid_0 // (num_kv_heads * max_seqlen_q) # [0, batch_size)
    pid_seq_q = pid_0 % max_seqlen_q # [0, max_seqlen_q)
    pid_k_heads = (pid_0 % (num_kv_heads * max_seqlen_q)) // max_seqlen_q # [0, num_k_heads)
    pid_q_heads = pid_k_heads * num_share_q_heads # [0, num_q_heads)

    # Process pid 1
    pid_split_k = tl.program_id(1) # [0, SPLIT_K)

    # Get q, k start and len
    q_start = tl.load(cu_seqlens_q_ptr + pid_batch)
    q_len = tl.load(cu_seqlens_q_ptr + pid_batch + 1) - q_start # q_len <= max_seqlen_q
    ...
    q = tl.load(q_ptrs, boundary_check=(0, 1), padding_option="zero") # [BLOCK_H, head_dim]

    # Init
    m_i = tl.full((BLOCK_H,), float('-inf'), dtype=tl.float32)
    lse_i = tl.full((BLOCK_H,), float("-inf"), dtype=tl.float32)
    acc_o = tl.zeros((BLOCK_H, head_dim), dtype=tl.float32)
    qk_scale = sm_scale * 1.44269504

    # Common offsets
    offs_block_size_k = tl.arange(0, block_size_k)
    offs_h_aligned = tl.arange(0, BLOCK_H)
    ...
    for i in range(start_topk, real_end_topk):
        kv_idx = tl.load(topk_idx_base_ptr + i * stride_tk) # [1]
        ...
        if kv_idx >= 0:
            ...
            if HAS_MASK:
                attention_mask_ptrs = tl.make_block_ptr(
                    ...
                    offsets=(pid_q_len, k_start + c),
                    block_shape=(1, block_size_k),
                    order=(1, 0),
                )
                attention_mask = tl.load(attention_mask_ptrs, boundary_check=(0, 1), padding_option="zero")
                qk = tl.where(attention_mask > 0, qk, float('-inf')) # broadcast attention_mask
    ...
    # Write back
    partial_o_ptrs = tl.make_block_ptr(
        base=partial_o_ptr + pid_q_len * stride_pol + pid_q_heads * stride_poh + pid_split_k * stride_pospk,
        shape=(num_share_q_heads, head_dim),
        strides=(stride_poh, stride_pod),
        offsets=(0, 0),
        block_shape=(BLOCK_H, head_dim),
        order=(1, 0),
    )
    tl.store(partial_o_ptrs, acc_o.to(partial_o_ptr.dtype.element_ty), boundary_check=(0, 1))

    partial_lse_ptrs = \
        partial_lse_ptr + \
        (pid_q_heads + offs_h_aligned) * stride_plh + \
        pid_q_len * stride_pll + \
        pid_split_k * stride_plspk
    mask_h = offs_h_aligned < num_share_q_heads
    tl.store(partial_lse_ptrs, lse_i, mask=mask_h)
...
def _topk_sparse_attention_fwd_v2(
