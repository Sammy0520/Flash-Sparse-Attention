...
@triton.jit
def forward_kernel_decode_split_k_fp8_quant(
    q_ptr, k_ptr,
    v_fp8_ptr, # [total_v_len, num_v_heads, head_dim]
    v_scale_ptr, # [total_v_len, num_v_heads]
    ...
    v_fp8_ptrs = tl.make_block_ptr(
        base=v_fp8_ptr + k_start * stride_vn + pid_kh * stride_vh,
    ...
    v_scale_ptrs = tl.make_block_ptr(
        base=v_scale_ptr + k_start * stride_vsn + pid_kh * stride_vsh,
        shape=(k_len, 1),
        strides=(stride_vsn, 1),
        offsets=(shard_lo, 0),
    ...
    for i in range(shard_lo, shard_hi, BLOCK_SIZE_K):
        ...
        v_fp8 = tl.load(v_fp8_ptrs, boundary_check=(0, 1))
        v_scale = tl.load(v_scale_ptrs, boundary_check=(0, 1), padding_option="zero")
        v = (v_fp8.to(tl.float32) * v_scale.to(tl.float32)).to(q.dtype)
        #v = v_fp8.to(q.dtype)
        ...
        v_fp8_ptrs = tl.advance(v_fp8_ptrs, (BLOCK_SIZE_K, 0))
        v_scale_ptrs = tl.advance(v_scale_ptrs, (BLOCK_SIZE_K, 0))
...
def _compressed_attention_fwd_decode(
    ...
    forward_kernel_decode_split_k_fp8_quant[grid_fwd](
        q, k,
        v_fp8, v_scale,
        ...
        stride_vn=v_fp8.stride(0),
        stride_vh=v_fp8.stride(1),
        stride_vd=v_fp8.stride(2),
        stride_vsn=v_scale.stride(0),
        stride_vsh=v_scale.stride(1),
