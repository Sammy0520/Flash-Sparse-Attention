...
def main():
    ...
    # The base content derived from your input
    base_para = ("Speculative decoding accelerates inference by verifying multiple "
    ...
    # Technical "Expansion" content to simulate real-world documentation
    technical_context = """
    [Technical Deep-Dive]
    The synergy between Speculative Decoding and Native Sparse Attention (NSA) targets 
    the two primary bottlenecks of LLM inference: compute-bound verification and 
    memory-bound long-context management. 

    1. Speculative Mechanism: By leveraging a smaller draft model $M_{draft}$, we generate 
    a look-ahead window of size $k$. The target model $M_{target}$ then performs a 
    single forward pass to validate these $k$ tokens in parallel using a modified 
    Rejection Sampling scheme where $P(x)$ is the target distribution.

    2. NSA Implementation: Unlike post-hoc pruning, Native Sparse Attention utilizes 
    hardware-aware kernels (e.g., Triton or FlashAttention-3) to skip non-essential 
    blocks in the $QK^T$ matrix. This reduces the KV Cache footprint from $O(L^2)$ 
    to $O(L \cdot \text{block\_size})$, which is critical for 128k+ sequences.
    """

    # --- THE PREFILL CONSTRUCTION ---
    # Adjust 'multiplier' to reach your desired token count
    # multiplier = 50 is roughly 10k tokens; 500 is roughly 100k tokens.
    multiplier = 512

    para = (
        f"SYSTEM INSTRUCTION: You are a senior AI researcher. Analyze the following text.\n\n"
        f"CORE THESIS: {base_para}\n\n"
        f"--- START OF CONTEXT ---\n"
        f"{ (technical_context + base_para) * multiplier }"
        f"\n--- END OF CONTEXT ---\n\n"
        f"FINAL TASK: Based on the 100+ references above, explain how the 'Rejection Sampling' "
        f"in Speculative Decoding handles the 'Sparse KV Cache' pointers of NSA."
    )
    ...
    t_once = benchmark(
        lambda: nsa_layer(hidden_batch, k_cache, v_cache, cmp_k, cmp_v,
                          cu_q, cu_k, position_ids=pos, use_splitk_impl=False))

    t_1xN = benchmark(
        lambda: [nsa_layer(hidden_batch[i:i+1], k_cache, v_cache, cmp_k, cmp_v,
                           torch.tensor([0, 1], device=device, dtype=torch.int32),
                           cu_k, position_ids=pos[i:i+1], use_splitk_impl=False) for i in range(N)])
    ...
    t_splitk = benchmark(
        lambda: nsa_layer(hidden_batch, k_cache, v_cache, cmp_k, cmp_v,
                          cu_q, cu_k, position_ids=pos, use_splitk_impl=True))
    print(f"  {N}-token once (splitk): {t_splitk:.3f} ms")
    print(f"  Speedup (splitk/ref):   {t_once / t_splitk:.2f}×")
    print(f"  Speedup (splitk/1×N):   {t_1xN / t_splitk:.2f}×")
